...
# API URL from environment variable1111
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                flash(
                    response.json().get("message", "Failed to update settings"), "error"
                )
        except Exception as e:
            flash(f"Error: {str(e)}", "error")
...
def answer_question():
    ...
    try:
        ...
        if result.status_code == 200:
            ...
            flash(
                result.json().get(
                    "message", f"Failed to submit response: Status {result.status_code}"
                ),
                "error",
            )
    except Exception as e:
...
def add_event():
    ...
    try:
        response = api_request(
            ...
            method="POST",
            token=session["token"],
            data={"title": title, "startTime": start_time, "endTime": end_time},
        )
...
def send_message():
    ...
    try:
        ...
        response = api_request(
            "messages/send", method="POST", token=session["token"], data=data
        )
...
def scheduled_messages():
    ...
    return render_template(
        "scheduled_messages.html", scheduled_messages=scheduled_messages
    )
...
def cancel_scheduled_message(message_id):
    ...
    try:
        response = api_request(
            f"messages/scheduled/{message_id}/cancel",
            method="POST",
            token=session["token"],
        )
        if response.status_code == 200:
            ...
            flash(
                response.json().get("message", "Failed to cancel scheduled message"),
                "error",
            )
    except Exception as e:
        flash(f"Error: {str(e)}", "error")
...
def accept_invite():
    ...
    try:
        response = api_request(
            "auth/partner/accept", method="POST", token=session["token"]
        )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            error_msg = response.json().get(
                "message", "Failed to disconnect partnership"
            )
            flash(error_msg, "error")
    except Exception as e:
        flash(f"Error: {str(e)}", "error")
...
def update_profile():
    ...
    try:
        response = api_request(
            "auth/profile", method="PUT", token=session["token"], data={"name": name}
        )
