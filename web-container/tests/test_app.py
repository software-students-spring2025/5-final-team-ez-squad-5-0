...
def test_register_post_success(client):
    with patch('app.api_request') as mock_api:
        ...
        mock_api.return_value.json.return_value = {"message": "User registered successfully"}
...
def test_register_post_failure(client):
    with patch('app.api_request') as mock_api:
        ...
        mock_api.return_value.json.return_value = {"message": "Email already registered"}
...
def test_settings_post(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 200
        mock_api.return_value.json.return_value = {"message": "Settings updated successfully"}
    ...
    with patch('app.api_request') as mock_api:
        ...
        def side_effect(endpoint, *args, **kwargs):
            ...
            if endpoint == 'calendar/events':
                ...
                mock_response.status_code = 200
                mock_response.json.return_value = {"question": "What made you smile today?"}
            elif endpoint == 'daily-question/answers':
                mock_response.status_code = 200
                mock_response.json.return_value = {"answers": [
                    {"user_name": "Partner", "answer": "Good coffee!"}
                ]}
            else:
...
def test_dashboard_question_submit(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 200
        mock_api.return_value.json.return_value = {"message": "Answer submitted successfully"}
        
        response = client.post('/dashboard/question', data={
            'response': 'My answer to the daily question'
        }, follow_redirects=True)
...
def test_calendar_page(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 200
        mock_api.return_value.json.return_value = {"events": [
...
def test_add_event(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 201
        mock_api.return_value.json.return_value = {"message": "Event created successfully"}
        
        response = client.post('/calendar/add', data={
            'title': 'New Event',
...
def test_send_message(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 201
        mock_api.return_value.json.return_value = {"message": "Message sent successfully"}
...
def test_schedule_message(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 201
        mock_api.return_value.json.return_value = {"message": "Message scheduled successfully"}
        
        response = client.post('/messages/schedule', data={
...
def test_scheduled_messages_page(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 200
        mock_api.return_value.json.return_value = {"scheduled_messages": [
            {
...
def test_cancel_scheduled_message(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 200
        mock_api.return_value.json.return_value = {"message": "Scheduled message cancelled successfully"}
...
def test_partner_page(client):
    ...
    with patch('app.api_request') as mock_api:
        ...
        mock_api.return_value.json.return_value = {
            "status": "connected",
            "partner": {"id": "partner123", "name": "Partner User", "email": "partner@example.com"}
        }
        ...
        assert b'connected' in response.data.lower() or b'partner' in response.data.lower()
...
def test_send_partner_invite(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 200
        mock_api.return_value.json.return_value = {"message": "Partnership invitation sent successfully"}
        
        response = client.post('/partner/send-invite', data={
            'partner_email': 'newpartner@example.com'
        }, follow_redirects=True)
...
def test_accept_partner_invite(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 200
        mock_api.return_value.json.return_value = {"message": "Partnership accepted successfully"}
...
def test_reject_partner_invite(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 200
        mock_api.return_value.json.return_value = {"message": "Partnership invitation rejected"}
...
def test_update_profile(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 200
        mock_api.return_value.json.return_value = {"message": "Profile updated successfully"}
...
def test_change_password(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 200
        mock_api.return_value.json.return_value = {"message": "Password changed successfully"}
...
def test_quiz_api_proxy_get(client):
    ...
    with patch('app.requests.get') as mock_get:
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.content = json.dumps({"questions": ["Question 1", "Question 2"]}).encode()
...
def test_disconnect_partner(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 200
        mock_api.return_value.json.return_value = {"message": "Partnership disconnected successfully"}
...
def test_dashboard_question_submit_api_failure(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 400
        mock_api.return_value.json.return_value = {"message": "Failed to submit response"}
...
def test_cancel_scheduled_message_api_error(client):
    ...
    with patch('app.api_request') as mock_api:
        ...
        response = client.post('/messages/scheduled/msg123/cancel', follow_redirects=True)
...
def test_reject_invite_api_failure(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 400
        mock_api.return_value.json.return_value = {"message": "Failed to reject invitation"}
...
def test_update_profile_api_failure(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 400
        mock_api.return_value.json.return_value = {"message": "Failed to update profile"}
...
def test_settings_api_failure(client):
    ...
    with patch('app.api_request') as mock_api:
        mock_api.return_value.status_code = 400
        mock_api.return_value.json.return_value = {"message": "Failed to update settings"}
