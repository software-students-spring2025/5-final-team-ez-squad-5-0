...
def auth_token(app):
    ...
    with patch('app.routes.get_user_by_id') as mock_get_user:
        mock_get_user.return_value = dict(TEST_USER)  # Use a copy to avoid modifying the original
...
def test_register_with_partner(client):
    ...
    with patch('app.mongo.db.users.find_one') as mock_find_one, \
         patch('app.mongo.db.users.insert_one') as mock_insert_one, \
         patch('app.mongo.db.users.update_one') as mock_update_one:

        # Correct mock for user and partner
        mock_find_one.side_effect = [
            None,  # First find for user email => new user
            {'_id': ObjectId('507f1f77bcf86cd799439012'), 'email': 'partner@example.com'},  # Partner found
            {'_id': ObjectId('507f1f77bcf86cd799439011')}  # Finally the newly created user
        ]
...
def test_login_invalid_credentials(client):
    ...
    with patch('app.routes.get_user_by_email') as mock_get_user:
        mock_get_user.return_value = dict(TEST_USER)  # Return a dictionary, not a MagicMock
    ...
    with patch('app.routes.get_user_by_id') as mock_get_user:
        ...
        with patch('app.routes.get_user_by_email') as mock_get_user_email:
            ...
            with patch('app.mongo.db.users.update_one') as mock_update:
                ...
                # Make request
                response = client.post('/api/auth/partner/invite', 
...
def test_accept_partner_invite(client, auth_token):
    ...
    with patch('app.routes.get_user_by_id') as mock_get_user:
        ...
        with patch('app.mongo.db.users.update_one') as mock_update:
            ...
            # Make request
            response = client.post('/api/auth/partner/accept', 
                                headers={'Authorization': f'Bearer {auth_token}'})
...
def test_reject_partner_invite(client, auth_token):
    ...
    with patch('app.routes.get_user_by_id') as mock_get_user:
        ...
        with patch('app.mongo.db.users.update_one') as mock_update:
            ...
            # Make request
            response = client.post('/api/auth/partner/reject', 
                                headers={'Authorization': f'Bearer {auth_token}'})
...
def test_get_partner_status(client, auth_token):
    ...
    with patch('app.routes.get_user_by_id') as mock_get_user:
        ...
        # Make request
        response = client.get('/api/auth/partner/status', 
                            headers={'Authorization': f'Bearer {auth_token}'})
...
def test_update_email_notifications(client, auth_token):
    ...
    with patch('app.mongo.db.users.update_one') as mock_update:
        ...
        # Make request
        response = client.put('/api/auth/notifications/email', 
                            json={'enabled': False},
                            headers={'Authorization': f'Bearer {auth_token}'})
...
def test_schedule_message(client, auth_token):
    ...
    with patch('app.mongo.db.scheduled_messages.insert_one') as mock_insert:
        ...
        # Make request
        response = client.post('/api/messages/schedule', 
                            json=message_data,
                            headers={'Authorization': f'Bearer {auth_token}'})
...
def test_get_scheduled_messages(client, auth_token):
    ...
    with patch('app.mongo.db.scheduled_messages.find') as mock_find:
        ...
        # Make request
        response = client.get('/api/messages/scheduled', 
                            headers={'Authorization': f'Bearer {auth_token}'})
...
def test_get_daily_question(client, auth_token):
    ...
    with patch('app.mongo.db.daily_questions.find_one') as mock_find:
        ...
        response = client.get('/api/daily-question/', 
                            headers={'Authorization': f'Bearer {auth_token}'})
            ...
            with patch('app.mongo.db.quiz_responses.count_documents') as mock_count:
                ...
                with patch('app.mongo.db.quiz_responses.find') as mock_find_responses:
                    mock_find_responses.return_value = [
                        {'question_id': 1, 'answer': 'Option A', 'user_id': TEST_USER['_id']},
                        {'question_id': 2, 'answer': 'Option B', 'user_id': TEST_USER['_id']},
                        {'question_id': 3, 'answer': 'Option C', 'user_id': TEST_USER['_id']}
                    ]
                    ...
                    with patch('app.mongo.db.quiz_responses.find_one') as mock_find_one_response:
                        ...
                        # Make request
                        response = client.get('/api/quiz/score', 
                                            headers={'Authorization': f'Bearer {auth_token}'})
...
def test_get_quiz_batch(client, auth_token):
    ...
    with patch('app.routes.get_user_by_id') as mock_get_user:
        ...
        with patch('app.mongo.db.quiz_batches.find_one') as mock_find_batch:
            ...
            # Make request
            response = client.get('/api/quiz/batch', 
                                headers={'Authorization': f'Bearer {auth_token}'})
    ...
    with patch('app.routes.get_user_by_id') as mock_get_user:
        ...
        with patch('app.mongo.db.quiz_batches.update_many') as mock_update:
            ...
            with patch('app.mongo.db.quiz_batches.find_one') as mock_find_batch:
                ...
                # Make request
                response = client.post('/api/quiz/batch/new', 
                                    headers={'Authorization': f'Bearer {auth_token}'})
        ...
        with patch('app.mongo.db.quiz_batches.find_one') as mock_find_batch:
            ...
            with patch('app.mongo.db.quiz_responses.find_one') as mock_find_response:
                ...
                with patch('app.mongo.db.quiz_scores.find_one') as mock_find_score:
                    ...
                    with patch('app.mongo.db.quiz_scores.update_one') as mock_update_score:
                        ...
                        with patch('app.mongo.db.quiz_responses.insert_one') as mock_insert:
                            ...
                            with patch('app.mongo.db.quiz_batches.update_one') as mock_update_batch:
                                mock_update_batch.return_value = MagicMock(modified_count=1)
                                
                                # Make request
                                response = client.post('/api/quiz/answer', 
                                                    json={
                                                        'question_id': 3,
                                                        'answer': 'E'
                                                    },
        ...
        with patch('app.mongo.db.quiz_responses.find_one') as mock_find_response:
            ...
            with patch('app.mongo.db.quiz_batches.find_one') as mock_find_batch:
                ...
                with patch('app.mongo.db.quiz_scores.find_one') as mock_find_score:
                    ...
                    # Make request
                    response = client.get('/api/quiz/check-partner-response?question_id=3', 
                                        headers={'Authorization': f'Bearer {auth_token}'})
...
def test_quiz_question_by_id(client, auth_token):
    ...
    with patch('app.mongo.db.quiz_questions.find_one') as mock_find:
        ...
        # Make request
        response = client.get(f'/api/quiz/question/{question_id}', 
                            headers={'Authorization': f'Bearer {auth_token}'})
