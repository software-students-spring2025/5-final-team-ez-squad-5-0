...
def test_send_async_email_exception(app):
    ...
    with patch(
        "app.email_utils.mail.send", side_effect=Exception("Test exception")
...
def test_send_email_html_only(app):
    ...
    with app.app_context():
        with patch("app.email_utils.Thread") as mock_thread, patch(
            ...
            send_email(
                "HTML Only Subject", ["recipient@example.com"], "<p>HTML Content</p>"
            )
...
def test_send_invitation_email(app):
    ...
    with patch("app.email_utils.send_email") as mock_send_email:
        ...
        assert (
            "Test Inviter would like to connect with you" in args[2]
        )  # content in html
        assert "Test Inviter has invited you to join Together" in args[3]  # text_body
        assert (
            "Test Inviter would like to connect with you" in args[3]
        )  # content in text
