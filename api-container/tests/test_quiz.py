...
class TestQuizModule(unittest.TestCase):
    ...
    def test_submit_answer_missing_params(self):
        response = self.client.post('/api/quiz/answer', headers=self.auth_headers, json={})
        self.assertEqual(response.status_code, 400)

    def test_submit_answer_no_partner(self):
        self.mock_users.find_one.return_value = {'_id': ObjectId()}
        response = self.client.post('/api/quiz/answer', headers=self.auth_headers, json={'question_id': 1, 'answer': 'A'})
        self.assertEqual(response.status_code, 400)

    def test_check_partner_response_missing_params(self):
        response = self.client.get('/api/quiz/check-partner-response', headers=self.auth_headers)
        self.assertEqual(response.status_code, 400)

    def test_check_partner_response_no_partner(self):
        self.mock_users.find_one.return_value = {'_id': ObjectId()}
        response = self.client.get('/api/quiz/check-partner-response?question_id=1', headers=self.auth_headers)
        self.assertEqual(response.status_code, 400)
