...
def test_get_profile_success(client, app, auth_headers, valid_object_id):
    ...
    with app.app_context():
        with patch('app.routes.settings.get_jwt_identity', return_value=valid_object_id), \
...
def test_update_email_notifications_enabled(client, app, auth_headers, valid_object_id):
    ...
    with app.app_context():
        with patch('app.routes.settings.get_jwt_identity', return_value=valid_object_id), \
             patch('app.routes.settings.mongo.db.users.update_one', return_value=mock_update_result) as mock_update:
            response = client.put(
...
def test_update_email_notifications_disabled(client, app, auth_headers, valid_object_id):
...
def test_update_email_notifications_no_changes(client, app, auth_headers, valid_object_id):
    ...
    with app.app_context():
        with patch('app.routes.settings.get_jwt_identity', return_value=valid_object_id), \
             patch('app.routes.settings.mongo.db.users.update_one', return_value=mock_update_result):
            response = client.put(
                '/api/settings/email-notifications',
                headers={**auth_headers, 'Content-Type': 'application/json'},
                data=json.dumps({'enabled': True})
...
def test_update_email_notifications_default_value(client, app, auth_headers, valid_object_id):
